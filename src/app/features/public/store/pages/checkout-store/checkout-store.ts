...
export class CheckoutStore implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<any>('assets/1_ubigeo_departamentos.json')
      .subscribe(res => {
        this.departamentos = res.ubigeo_departamentos;
      });

    this.http.get<any>('assets/2_ubigeo_provincias.json')
      .subscribe(res => {
        this.provincias = res.ubigeo_provincias;
      });
  ...
  buscarEnMapa(): void {
    if (this.direccion() && this.departamento() && this.provincia() &&
      ...
      const direccionCompleta = `${this.direccion()} ${this.numeroCasa()}, ${this.distrito()}, ${this.provincia()}, ${this.departamento()}, Perú`;

      const url = `https://maps.google.com/maps?q=${encodeURIComponent(direccionCompleta)}&t=&z=17&ie=UTF8&iwloc=&output=embed`;
